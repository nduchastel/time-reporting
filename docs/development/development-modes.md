...
```
           ┌──────────────┐
           │   Browser    │  Worker records voice via MediaRecorder API
           │ (localhost:  │
           │    5173)     │
           └──────┬───────┘
                  ...
                  ▼
           ┌──────────────┐
           │   Backend    │  Node.js Express - receives audio file
           │ (localhost:  │
           │    3001)     │
           └──────┬───────┘
                  │
     ...
     └─────┬────┘  └─────┬────┘
           ...
           └──────┬──────┘
                  ▼
           ┌──────────────┐
           │  Supabase    │  Store time card after Submit
           │     DB       │
           │ (Postgres)   │
           └──────────────┘
```
...
```
                ┌──────────────┐
                │  Test Suite  │  Vitest unit tests - run test scenarios
                │  (npm test)  │
                └──────┬───────┘
                       │
                       ▼
         ┌─────────────────────────────┐
         │  Mock Transcription Data    │  17 test scenarios:
         │  backend/tests/fixtures/    │  - Good/bad/edge cases
         │                             │  - Multiple languages
         │  transcriptions.js          │
         └──────────┬──────────────────┘
                    │
